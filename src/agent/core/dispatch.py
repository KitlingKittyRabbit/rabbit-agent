"""派发器：call_subagent / ask / answer_task 工具，后台任务表，中断取消。

任务生命周期：dispatch → running → done/error/cancelled。
...
class Dispatcher:
    ...
    def cancel_all(self) -> None:
        """中断所有运行中的任务（各自走 cancelled 分支发事件）。"""
        for task_id, task in self._running.items():
            if self.tasks.get(task_id) == "running":
                task.handle.cancel()
